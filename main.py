...
def Login():
    ...
    submit = Button(frame1, text="Submit",
                    command=lambda: check(userentry.get(), submit))
    submit.grid(row=2, column=2, pady=(15, 0), padx=(0, 30))


def check(username, delet):
    delet.destroy()
    usernameLst = []
    passwordLst = []
    upasswordLst = []
...
def storePass(inp):
    if inp not in inpPass:
        inpPass.append(inp)


def checkPass(actPass, cpass):
    scheckPass = ''
    for i in inpPass:
        scheckPass += i
    print(scheckPass)
    ...
    for j in actPass:
        if j in cpass:
            cfpass += j
    print(cfpass)
    if cfpass == scheckPass:
        messagebox.showinfo('Success', 'Password is correct.')
    else:
        messagebox.showerror('Wrong', 'Password entered is incorrect.')
...
def graphicPass(username, password):
    ...
    save = random.choice(list1)
    all = [password[0], difference[1], password[2], difference[3], password[4], difference[5],  password[6], difference[7], password[3], difference[9],
           password[1], difference[11], difference[12], difference[13], difference[14], difference[15]]
    cpass = [password[0], password[2], password[4],
             password[6], password[3], password[1]]

    pics = ['1.jpg', '2.jpg', '3.jpg', '4.jpg', '5.jpg']
    random.shuffle(pics)
    img1 = ImageTk.PhotoImage(Image.open(
        all[0] + "/" + pics[0]).resize((70, 70)))
    img2 = ImageTk.PhotoImage(Image.open(
        all[1] + "/" + pics[0]).resize((70, 70)))
    img3 = ImageTk.PhotoImage(Image.open(
        all[2] + "/" + pics[0]).resize((70, 70)))
    img4 = ImageTk.PhotoImage(Image.open(
        all[3] + "/" + pics[0]).resize((70, 70)))
    img5 = ImageTk.PhotoImage(Image.open(
        all[4] + "/" + pics[0]).resize((70, 70)))
    img6 = ImageTk.PhotoImage(Image.open(
        all[5] + "/" + pics[0]).resize((70, 70)))
    img7 = ImageTk.PhotoImage(Image.open(
        all[6] + "/" + pics[0]).resize((70, 70)))
    img8 = ImageTk.PhotoImage(Image.open(
        all[7] + "/" + pics[0]).resize((70, 70)))
    img9 = ImageTk.PhotoImage(Image.open(
        all[8] + "/" + pics[0]).resize((70, 70)))
    img10 = ImageTk.PhotoImage(Image.open(
        all[9] + "/" + pics[0]).resize((70, 70)))
    img11 = ImageTk.PhotoImage(Image.open(
        all[10] + "/" + pics[0]).resize((70, 70)))
    img12 = ImageTk.PhotoImage(Image.open(
        all[11] + "/" + pics[0]).resize((70, 70)))
    img13 = ImageTk.PhotoImage(Image.open(
        all[12] + "/" + pics[0]).resize((70, 70)))
    img14 = ImageTk.PhotoImage(Image.open(
        all[13] + "/" + pics[0]).resize((70, 70)))
    img15 = ImageTk.PhotoImage(Image.open(
        all[14] + "/" + pics[0]).resize((70, 70)))
    img16 = ImageTk.PhotoImage(Image.open(
        all[15] + "/" + pics[0]).resize((70, 70)))
    images = [img1, img2, img3, img4, img5, img6, img7, img8,
              img9, img10, img11, img12, img13, img14, img15, img16]
    random.Random(save).shuffle(images)
    random.Random(save).shuffle(all)
    b1 = customtkinter.CTkButton(
        frame2, image=images[0], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[0]))
    b2 = customtkinter.CTkButton(
        frame2, image=images[1], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[1]))
    b3 = customtkinter.CTkButton(
        frame2, image=images[2], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[2]))
    b4 = customtkinter.CTkButton(
        frame2, image=images[3], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[3]))
    b5 = customtkinter.CTkButton(
        frame2, image=images[4], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[4]))
    b6 = customtkinter.CTkButton(
        frame2, image=images[5], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[5]))
    b7 = customtkinter.CTkButton(
        frame2, image=images[6], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[6]))
    b8 = customtkinter.CTkButton(
        frame2, image=images[7], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[7]))
    b9 = customtkinter.CTkButton(
        frame2, image=images[8], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[8]))
    b10 = customtkinter.CTkButton(
        frame2, image=images[9], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[9]))
    b11 = customtkinter.CTkButton(
        frame2, image=images[10], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[10]))
    b12 = customtkinter.CTkButton(
        frame2, image=images[11], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[11]))
    b13 = customtkinter.CTkButton(
        frame2, image=images[12], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[12]))
    b14 = customtkinter.CTkButton(
        frame2, image=images[13], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[13]))
    b15 = customtkinter.CTkButton(
        frame2, image=images[14], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[14]))
    b16 = customtkinter.CTkButton(
        frame2, image=images[15], text="", padx=10, pady=10, height=70, width=70, command=lambda: storePass(all[15]))
    ...
    submit = Button(frame2, text="Submit",
                    command=lambda: checkPass(copyPass, cpass))
